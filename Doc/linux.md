...
private func createBootLoader() -> VZBootLoader {
    if File.exists(dir.dir.appendingPathComponent("vmlinuz")) {
        Logger.info("use linux kernel boot loader")
        let bootLoader = VZLinuxBootLoader(kernelURL: dir.dir.appendingPathComponent("vmlinuz"))
        bootLoader.initialRamdiskURL = dir.dir.appendingPathComponent("initrd.img")
        bootLoader.commandLine = "console=hvc0 root=/dev/vda3 ro modules=sd-mod,usb-storage,ext4 quiet rootfstype=ext4"
        return bootLoader
    }
    ...
}
...
func createConsole() -> VZSerialPortConfiguration {
    ...
    ttyHandle = FileHandle.init(forUpdatingAtPath: name)
